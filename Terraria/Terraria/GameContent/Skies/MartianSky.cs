    ...
    internal class MartianSky : CustomSky
    {
        ...
        public override void Update()
        {
            if (Main.gamePaused || !Main.hasFocus)
                return;
            int index1 = this._activeUfos;
            for (int index2 = 0; index2 < this._ufos.Length; ++index2)
            {
                MartianSky.Ufo ufo = this._ufos[index2];
                if (ufo.IsActive)
                {
                    ++ufo.Frame;
                    if (!ufo.Update())
                ...
                this._ufos[index2] = ufo;
            }
            if (!this._leaving && index1 != this._maxUfos)
            {
                this._ufos[index1].IsActive = true;
                this._ufos[index1++].AssignNewBehavior();
            }
            this._active = !this._leaving || index1 != 0;
            this._activeUfos = index1;
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            ...
            {
                float num3 = this._ufos[index].Depth;
