...
namespace Terraria.GameContent.Skies
{
    internal class SolarSky : CustomSky
    {
        ...
        public override void Update()
        {
            ...
            {
                this._meteors[index].Position.X -= num;
                this._meteors[index].Position.Y += num;
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            ...
            for (int index = 0; index < this._meteors.Length; ++index)
            {
                float num3 = this._meteors[index].Depth;
            ...
            for (int index = num1; index < num2; ++index)
            {
                ...
                int num3 = this._meteors[index].FrameCounter / 3;
                this._meteors[index].FrameCounter = (this._meteors[index].FrameCounter + 1) % 12;
                if (rectangle.Contains((int)position.X, (int)position.Y))
                    spriteBatch.Draw(this._meteorTexture, position, new Rectangle?(new Rectangle(0, num3 * (this._meteorTexture.Height / 4), this._meteorTexture.Width, this._meteorTexture.Height / 4)), Color.White * num4 * this._fadeOpacity, 0.0f, Vector2.Zero, vector2_1.X * 5f * this._meteors[index].Scale, SpriteEffects.None, 0.0f);
            }
        }
        ...
        internal override void Activate(Vector2 position, params object[] args)
        {
            ...
            for (int index = 0; index < this._meteors.Length; ++index)
            {
                ...
                this._meteors[index].Position.X = (float)((double)num * ((double)Main.maxTilesX * 16.0) + (double)Utils.NextFloat(this._random) * 40.0 - 20.0);
                this._meteors[index].Position.Y = (float)((double)Utils.NextFloat(this._random) * -(Main.worldSurface * 16.0 + 10000.0) - 10000.0);
                this._meteors[index].Depth = this._random.Next(3) == 0 ? (float)((double)Utils.NextFloat(this._random) * 5.0 + 4.80000019073486) : (float)((double)Utils.NextFloat(this._random) * 3.0 + 1.79999995231628);
                this._meteors[index].FrameCounter = this._random.Next(12);
                this._meteors[index].Scale = (float)((double)Utils.NextFloat(this._random) * 0.5 + 1.0);
