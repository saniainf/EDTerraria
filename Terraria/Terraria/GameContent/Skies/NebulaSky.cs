...
namespace Terraria.GameContent.Skies
{
    internal class NebulaSky : CustomSky
    {
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            ...
            {
                float num3 = this._pillars[index].Depth;
            ...
            for (int index1 = num1; index1 < num2; ++index1)
            {
                ...
                if (rectangle.Contains((int)position.X, (int)position.Y))
                {
                    ...
                    spriteBatch.Draw(this._beamTexture, position, new Rectangle?(), Color.White * 0.2f * num4 * this._fadeOpacity, 0.0f, Vector2.Zero, new Vector2(num3 / 70f, num3 / 45f), SpriteEffects.None, 0.0f);
                    int index2 = 0;
                    float num5 = 0.0f;
                    while ((double)num5 <= 1.0)
                    {
                        ...
                        spriteBatch.Draw(this._rockTextures[index2], position + new Vector2((float)(Math.Sin((double)num5 * 1582.0) * ((double)num3 * 0.5) + (double)num3 * 0.5), num6 * 2000f), new Rectangle?(), Color.White * num6 * num4 * this._fadeOpacity, num6 * 20f, new Vector2((float)(this._rockTextures[index2].Width >> 1), (float)(this._rockTextures[index2].Height >> 1)), 0.9f, SpriteEffects.None, 0.0f);
        ...
        internal override void Activate(Vector2 position, params object[] args)
        {
            ...
            for (int index = 0; index < this._pillars.Length; ++index)
            {
                this._pillars[index].Position.X = (float)((double)index / (double)this._pillars.Length * ((double)Main.maxTilesX * 16.0 + 20000.0) + (double)Utils.NextFloat(this._random) * 40.0 - 20.0 - 20000.0);
                this._pillars[index].Position.Y = (float)((double)Utils.NextFloat(this._random) * 200.0 - 2000.0);
                this._pillars[index].Depth = (float)((double)Utils.NextFloat(this._random) * 8.0 + 7.0);
            }
