...
namespace Terraria.GameContent.Generation
{
    internal class PassLegacy : GenPass
    {
        ...
        private static Dictionary<string, float> GenerateWeightMap()
        {
            ...
            foreach (string str2 in str1.Split(new char[1] { ',' }))
            {
                string[] strArray = str2.Split(new char[1] { ':' });
                dictionary[strArray[0]] = float.Parse(strArray[1], NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture);
            }
        ...
        {
            float num;
            if (!_weightMap.TryGetValue(name, out num))
                num = 1f;
