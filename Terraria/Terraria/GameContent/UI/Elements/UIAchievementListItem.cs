...
namespace Terraria.GameContent.UI.Elements
{
    internal class UIAchievementListItem : UIPanel
    {
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            Vector2 stringSize1 = ChatManager.GetStringSize(Main.fontItemStack, this._achievement.Description, baseScale2, num);
            ...
            position1.Y += 2f;
            position1.X += 4f;
            spriteBatch.Draw(this._categoryTexture, position1, new Rectangle?(Utils.Frame(this._categoryTexture, 4, 2, (int)category, 0)), this.IsMouseHovering ? Color.White : Color.Silver, 0.0f, Vector2.Zero, 0.5f, SpriteEffects.None, 0.0f);
            position1.X += 4f;
            position1.X += 17f;
            ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, this._achievement.FriendlyName, position1, baseColor1, 0.0f, Vector2.Zero, baseScale1, num, 2f);
            position1.X -= 17f;
            Vector2 position2 = vector2 + Vector2.UnitY * 27f;
            this.DrawPanelBottom(spriteBatch, position2, num, color1);
            position2.X += 8f;
            position2.Y += 4f;
            ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, this._achievement.Description, position2, baseColor2, 0.0f, Vector2.Zero, baseScale2, num - 10f, 2f);
            if (!flag)
                return;
            ...
            this.DrawProgressBar(spriteBatch, progress, position3 - Vector2.UnitX * Width * 0.7f, Width, BackColor, color2, Utils.MultiplyRGBA(color2, new Color(new Vector4(1f, 1f, 1f, 0.5f))));
