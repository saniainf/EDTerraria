        ...
        public override bool Place(Point origin, StructureMap structures)
        {
            ...
            for (int index3 = 0; index3 < 300; ++index3)
            {
                for (int index4 = max1; index4 <= min1; ++index4)
                {
                    ...
                    {
                        GraniteBiome.Magma magma1 = GraniteBiome._sourceMagmaMap[index4, index5];
                        if (magma1.IsActive)
                        {
                            float num1 = 0.0f;
                            ...
                            for (int index6 = -1; index6 <= 1; ++index6)
                            {
                                for (int index7 = -1; index7 <= 1; ++index7)
                                {
                                    if (index6 != 0 || index7 != 0)
                                    {
                                        ...
                                        vector2.Normalize();
                                        GraniteBiome.Magma magma2 = GraniteBiome._sourceMagmaMap[index4 + index6, index5 + index7];
                                        if ((double)magma1.Pressure > 0.00999999977648258 && !magma2.IsActive)
                                        {
                                            ...
                                            GraniteBiome._targetMagmaMap[index4 + index6, index5 + index7] = magma2.ToFlow();
                                        }
                            ...
                            if ((double)num3 > (double)magma1.Resistance)
                            {
                                ...
                                float pressure = Math.Max(0.0f, (float)((double)Math.Max(num3 - num2 - magma1.Pressure, 0.0f) + (double)num2 + (double)magma1.Pressure * 0.875) - magma1.Resistance);
                                GraniteBiome._targetMagmaMap[index4, index5] = GraniteBiome.Magma.CreateFlow(pressure, Math.Max(0.0f, magma1.Resistance - pressure * 0.02f));
                            }
                        }
                    }
                }
            ...
            for (int index3 = max1; index3 <= min1; ++index3)
            {
                ...
                {
                    GraniteBiome.Magma magma = GraniteBiome._sourceMagmaMap[index3, index4];
                    if (magma.IsActive)
                    {
                        Tile tile = GenBase._tiles[origin.X + index3, origin.Y + index4];
                        if ((double)Math.Max(1f - Math.Max(0.0f, (float)(Math.Sin((double)(origin.Y + index4) * 0.400000005960464) * 0.699999988079071 + 1.20000004768372) * (float)(0.200000002980232 + 0.5 / Math.Sqrt((double)Math.Max(0.0f, magma.Pressure - magma.Resistance)))), magma.Pressure / 15f) > 0.349999994039536 + (WorldGen.SolidTile(origin.X + index3, origin.Y + index4) ? 0.0 : 0.5))
                        {
