    ...
    public static class Minecart
    {
        ...
        public static BitsByte TrackCollision(ref Vector2 Position, ref Vector2 Velocity, ref Vector2 lastBoost, int Width, int Height, bool followDown, bool followUp, int fallStart, bool trackOnly, Action<Vector2> MinecartDust)
        {
            ...
            while (true)
            {
                ...
                {
                    int index4 = trackState1 != Minecart.TrackState.OnBack ? (int)Minecart.FrontTrack(tileTrack) : (int)Minecart.BackTrack(tileTrack);
                ...
                if (flag1)
                {
                    ...
                    int index4 = (int)Minecart.FrontTrack(tileTrack);
                    int index5 = (int)Minecart.BackTrack(tileTrack);
                        ...
                        if (flag7 || (double)vector2_3.Y > 0.0)
                        {
                            switch (trackState1)
                            {
                                case Minecart.TrackState.AboveTrack:
                                    ...
                                    break;
                                case Minecart.TrackState.OnTrack:
                                    int num5 = Minecart._tileHeight[(int)Minecart.FrontTrack(tileTrack)][index3];
                                    int num6 = Minecart._tileHeight[(int)Minecart.BackTrack(tileTrack)][index3];
                            ...
                            switch (trackState2)
                            {
                                case Minecart.TrackState.OnTrack:
                                case Minecart.TrackState.OnFront:
                                    index4 = (int)Minecart.FrontTrack(tileTrack);
                                    break;
                                case Minecart.TrackState.OnBack:
                                    index4 = (int)Minecart.BackTrack(tileTrack);
                                    break;
                            }
                            if (index4 != -1)
                            {
                ...
                {
                    if (flag1 && Minecart._trackType[(int)Minecart.FrontTrack(tileTrack)] == 1)
                        flag3 = true;
                    vector2_3.Y = 0.0f;
                }
        ...
        public static bool FrameTrack(int i, int j, bool pound, bool mute = false)
        {
            ...
            int index3 = -1;
            int index4 = -1;
            int[] numArray = Minecart._trackSwitchOptions[index1];
            if (numArray == null)
            {
                if (pound)
                    return false;
                Minecart.FrontTrack(tileTrack, (short)0);
                Minecart.BackTrack(tileTrack, (short)-1);
                return false;
            }
            if (!pound)
            {
            ...
            else
            {
                ...
                for (int index5 = 0; index5 < numArray.Length; ++index5)
                {
                    if (Minecart._trackType[numArray[index5]] == num2)
                    {
                        if (Minecart._leftSideConnection[numArray[index5]] == -1 || Minecart._rightSideConnection[numArray[index5]] == -1)
                            ++num4;
                        else
                            ++num3;
                    }
                ...
                if (!flag1)
                {
                    ...
                    {
                        ++index4;
                        if (index4 >= numArray.Length)
                        {
                            index4 = -1;
                            break;
                        }
                        if ((Minecart._leftSideConnection[numArray[index4]] != Minecart._leftSideConnection[numArray[index3]] || Minecart._rightSideConnection[numArray[index4]] != Minecart._rightSideConnection[numArray[index3]]) && (Minecart._trackType[numArray[index4]] == num2 && Minecart._leftSideConnection[numArray[index4]] != -1) && Minecart._rightSideConnection[numArray[index4]] != -1)
                            flag2 = true;
                    }
                }
                if (!flag2)
                {
                    do
                    {
                        ...
                        if (index3 >= numArray.Length)
                        {
                            ...
                            while (Minecart._trackType[numArray[index3]] != num2 || (Minecart._leftSideConnection[numArray[index3]] == -1 ? 1 : (Minecart._rightSideConnection[numArray[index3]] == -1 ? 1 : 0)) != (flag1 ? 1 : 0));
                            break;
                        }
                    }
                    while (Minecart._trackType[numArray[index3]] != num2 || (Minecart._leftSideConnection[numArray[index3]] == -1 ? 1 : (Minecart._rightSideConnection[numArray[index3]] == -1 ? 1 : 0)) != (flag1 ? 1 : 0));
            ...
            if (index3 == -2)
            {
                if ((int)Minecart.FrontTrack(tileTrack) != (int)Minecart._firstPressureFrame)
                    flag3 = true;
            }
            else if (index3 == -1)
            {
                if ((int)Minecart.FrontTrack(tileTrack) != 0)
                    flag3 = true;
            }
            else if ((int)Minecart.FrontTrack(tileTrack) != numArray[index3])
                flag3 = true;
            if (index4 == -1)
            {
                if ((int)Minecart.BackTrack(tileTrack) != -1)
                    flag3 = true;
            }
            else if ((int)Minecart.BackTrack(tileTrack) != numArray[index4])
                flag3 = true;
            if (index3 == -2)
                Minecart.FrontTrack(tileTrack, Minecart._firstPressureFrame);
            else if (index3 == -1)
                Minecart.FrontTrack(tileTrack, (short)0);
            else
                Minecart.FrontTrack(tileTrack, (short)numArray[index3]);
            if (index4 == -1)
                Minecart.BackTrack(tileTrack, (short)-1);
            else
                Minecart.BackTrack(tileTrack, (short)numArray[index4]);
            if (pound && flag3 && !mute)
        ...
        public static void FlipSwitchTrack(int i, int j)
        {
            ...
            switch (Minecart._trackType[(int)trackID])
            {
                case 0:
                    if ((int)Minecart.BackTrack(tileTrack) == -1)
                        break;
                    Minecart.FrontTrack(tileTrack, Minecart.BackTrack(tileTrack));
                    Minecart.BackTrack(tileTrack, trackID);
