    ...
    public static class DelegateMethods
    {
        ...
        public static bool CutTiles(int x, int y)
        {
            if (!WorldGen.InWorld(x, y, 1) || Main.tile[x, y] == null)
                return false;
            if (!Main.tileCut[(int)Main.tile[x, y].type] || Main.tile[x, y + 1] == null || ((int)Main.tile[x, y + 1].type == 78 || (int)Main.tile[x, y + 1].type == 380))
                return true;
        ...
        public static class Minecart
        {
            ...
            {
                dustPosition += Utils.RotatedBy(new Vector2(Main.rand.Next(2) == 0 ? 13f : -13f, 0.0f), (double)DelegateMethods.Minecart.rotation, new Vector2());
