...
namespace Terraria
{
    public class Mount
    {
        ...
        public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
        {
            ...
            switch (this._type)
            {
                ...
                case 7:
                    if (drawType == 3)
                    {
                        drawColor = new Color(250, 250, 250, (int)byte.MaxValue);
                        break;
                    }
                    break;
                case 9:
            ...
            switch (this._type)
            {
                case 7:
                    ...
                    break;
                case 8:
                    Mount.DrillMountData drillMountData1 = (Mount.DrillMountData)this._mountSpecificData;
                    if (drawType == 0)
                    {
                        rotation = drillMountData1.outerRingRotation - rotation;
                        break;
                    }
                    if (drawType == 3)
                    {
                        rotation = drillMountData1.diodeRotation - rotation - drawPlayer.fullRotation;
                        break;
                    }
                    break;
            }
            ...
            if (!flag)
            {
                drawData = new DrawData(texture1, Position, new Rectangle?(rectangle1), drawColor, rotation, origin, scale1, effect, 0);
                drawData.shader = Mount.currentShader;
                playerDrawData.Add(drawData);
                if (texture2 != null)
                {
                    drawData = new DrawData(texture2, Position, new Rectangle?(rectangle1), color1, rotation, origin, scale1, effect, 0);
                    drawData.shader = Mount.currentShader;
                }
                playerDrawData.Add(drawData);
            }
            ...
            Vector2 vector2_1 = Utils.RotatedBy(Mount.drillDiodePoint1, (double)drillMountData2.diodeRotation, new Vector2());
            Vector2 vector2_2 = Utils.RotatedBy(Mount.drillDiodePoint2, (double)drillMountData2.diodeRotation, new Vector2());
            for (int index1 = 0; index1 < drillMountData2.beams.Length; ++index1)
            {
                Mount.DrillBeam drillBeam = drillMountData2.beams[index1];
        ...
        public bool CanMount(int m, Player mountingPlayer)
        {
            ...
            return Collision.IsClearSpotHack(mountingPlayer.position + new Vector2(0.0f, (float)(mountingPlayer.height - Height)), 2f, mountingPlayer.width, Height, false, false, 1, true, false);
        }
